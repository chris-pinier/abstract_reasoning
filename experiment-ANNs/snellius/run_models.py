...
def generate_text(prompt: str, max_new_tokens: int = MAX_NEW_TOKENS, temperature: float = 1.0, top_p: float = 1.0, top_k=None) -> str:
    """
    ...
    generated_text_cleaned = tokenizer.decode(generated_tokens, skip_special_tokens=True)
    ...
    outputs = dict(
        ...
        text_raw=generated_text_raw,
        text_cleaned=generated_text_cleaned,
    )

    return dict(inputs=inputs, outputs=outputs)


def run_model_2(model_id: str, prompt_file: Path, hook_layer_names:list):
    timestamp = get_timestamp()
    model_id_str = model_id.replace("/", "--")
    ...
    # * ---------------------------------------- *
    # TODO: Rewrite / reimplemnt code below (taken from run_models() )
    # hooked_layers_outputs, hook_handles = get_layer_activations(pipe, hook_layer_names)
