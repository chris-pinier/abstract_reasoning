...
from .query_utils import (
    get_completions,
    get_all_head_simmats,
    get_summed_vec_per_item,
...
class Acc:
    ...
    def completions(self):
        if not hasattr(self, "_completions"):
            self._completions = get_completions(self.model, self.datasets)
        return self._completions

    @completions.setter
    def completions(self, completions: List[str]):
        self._completions = completions

    def load_completions(self, path: str):
        with open(path, "r") as f:
            completions = [line.strip("\n") for line in f.readlines()]
        assert len(completions) == len(self.datasets.prompts), (
            f"Expected {len(self.datasets.prompts)} completions, got {len(completions)}"
        )
    ...
    @property
    def acc_df(self):
        if not hasattr(self, "_acc_df"):
            self._acc_df = self._create_acc_df()
        return self._acc_df
    ...
    def _create_acc_df(self):
        ...
        df["concept"] = df["dataset"].apply(
            lambda x: [
                d.task.concept for d in self.datasets.datasets if d.dataset_name == x
            ][0]
        )
        df["list"] = df["dataset"].apply(
            lambda x: [
                d.task.sequence_name
                for d in self.datasets.datasets
                if d.dataset_name == x
            ][0]
        )
        df["shuffle_prop"] = df["dataset"].apply(
            lambda x: [
                d.task.shuffle_prop
                for d in self.datasets.datasets
                if d.dataset_name == x
    ...
    def _evaluate(self):
        ...
        for i, dataset in enumerate(self.datasets.datasets):
            model_completions = self._completions[
                dataset.size * i : dataset.size * (i + 1)
            ]
            acc, correct = accuracy_completions(
                self.model, model_completions, dataset.completions, return_correct=True
            )
...
class RSA:
    def __init__(self, model: StandardizedTransformer, dataset: AbstractTask):
        self.model = model
    ...
    @simmats.setter
    def simmats(self, simmats: np.ndarray):
        self._simmats = simmats
    ...
    def top_heads(
        self, k: int = 1, return_rsa: bool = False
        ...
        if return_rsa:
            return [
                (int(layer), int(head), self.rsa[layer, head].item())
                for layer, head in [
                    np.unravel_index(idx, self.rsa.shape) for idx in indices
                ]
            ]
        return [
            (int(layer), int(head))
    ...
    def batch_rsa(self, simmats: torch.Tensor, design_matrix: np.ndarray) -> np.ndarray:
        ...
        for layer in tqdm(range(n_layers), desc="Computing RSA"):
            for head in range(n_components):
                rsa_results[layer][head] = compute_rsa(
                    design_matrix, simmats[layer, head]
                )
        rsa_results = np.nan_to_num(
            np.array(rsa_results), nan=0.0
    ...
    def plot_top_heads(self, k: int = 5, labels: List[str] = None):
        ...
        vmin = min([np.min(self.simmats[layer, head]) for layer, head in top_heads])
        vmax = max([np.max(self.simmats[layer, head]) for layer, head in top_heads])
    ...
    def load_rsa(self, path: str):
        self.rsa = np.load(path)
    ...
    def save_simmats(self, path: str, top_n: int = None):
        ...
        np.savez(
            path, simmats=top_simmats, indices=indices, full_shape=self.simmats.shape
        )
