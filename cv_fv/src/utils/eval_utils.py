...
class SimilarityMatrix:
    ...
    def to_task_level(self, exclude_diagonal: bool = True) -> 'SimilarityMatrix':
        '''
        Collapse the matrix into a task-level summary by averaging within
        each task block (diagonal) and between each pair of task blocks
        (off-diagonal).

        Args:
            exclude_diagonal: If True, exclude the main diagonal when
                computing within-task (on-block-diagonal) means.

        Returns:
            A new SimilarityMatrix of shape (n_tasks, n_tasks).
        '''
        unique_tasks = list(self.tasks_idx.keys())
        n = len(unique_tasks)
        task_mat = np.zeros((n, n))

        for i, task_i in enumerate(unique_tasks):
            s_i, e_i = self.tasks_idx[task_i]
            for j, task_j in enumerate(unique_tasks):
                s_j, e_j = self.tasks_idx[task_j]
                block = self.matrix[s_i:e_i+1, s_j:e_j+1]

                if i == j and exclude_diagonal:
                    mask = ~np.eye(block.shape[0], dtype=bool)
                    task_mat[i, j] = block[mask].mean()
                else:
                    task_mat[i, j] = block.mean()

        return SimilarityMatrix(sim_mat=task_mat, tasks=unique_tasks)
