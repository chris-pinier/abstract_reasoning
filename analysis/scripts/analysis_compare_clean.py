...
# * RELATIVE IMPORTS
# * ########################################
from data_loader.human_data import HumanGroupData
from analysis_ANNs import ANNGroupData
...
def main(eeg_chan_group: str, n_perm, n_boot):
    ...
    ann_data_dir = (
        DIRECTORIES.ann.analyzed / "RSA-seq_tokens-metric_correlation/best_layer"
    )

    humans_datasets, ANNs_datasets = combined_data.get_rsa_datasets(
    ...
    # * Compare Performance
    # * ================================================================================
    human_data_dir = DIRECTORIES.human.analyzed / "RSA-Accuracy"
    ann_data_dir = DIRECTORIES.ann.analyzed / "RSA-Accuracy"
    save_dir_perf = save_dir / "performance"

    combined_data.rsa_accuracy(
        human_data_dir=human_data_dir,
        ann_data_dir=ann_data_dir,
        save_dir=save_dir_perf,
    ...
    # ! TEMP 2
    self = combined_data
    humans_datasets, ANNs_dataset = self.get_rsa_datasets(
        human_data_dir=human_data_dir, ann_data_dir=ann_data_dir, level="pattern"
    )
    # ! TEMP
    # * ================================================================================
    ...
    combined_data.rsa_representations(
        human_data_dir=human_data_dir,
        ann_data_dir=ann_data_dir,
...
def multimodal_rsa():
    ...
    human_ds_repr, ann_ds_repr = get_rsa_datasets(
        human_data_dir_repr, ann_data_dir_repr, lvl
    )

    human_ds_acc, ann_ds_acc = get_rsa_datasets(
...
def temp_perf_and_repr_corr():
    ...
    for lvl in ["pattern"]:
        eeg_ds, ANNs_ds = get_rsa_representation_datasets(
            human_data_dir, ann_data_dir, lvl
        )
...
if __name__ == "__main__":
    ...
    # struct = DisplayTree(DIRECTORIES.human.data, stringRep=True)
    # print(struct)
    # re.findall(r"(.+)-behav.csv", struct)
